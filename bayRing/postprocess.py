...
def model_component_lists(results, inference_model, method=None):
    ...
    models_list       = [np.array(inference_model.model(p)) for p in parameter_samples]
    models_re_list    = [np.real(model) for model in models_list]
    models_im_list    = [np.imag(model) for model in models_list]
